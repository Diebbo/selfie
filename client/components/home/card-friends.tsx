...
export const CardFriends = ({
  ...
  const [userErrorMessage, setUserErrorMessage] = React.useState<string | null>(
    null,
  );
  ...
  React.useEffect(() => {
    const awaitUsernames = async () => {
      try {
        ...
        const filteredUsernames = peopleUsernames.filter(
          (user) =>
  ...
  const handleNewFriend = async () => {
    ...
    try {
      const friendToAdd = usersCasted.find(
        (user) => user.username === newFriendUser,
      );
      if (friendToAdd) {
        ...
        const filteredUsernames = usersCasted.filter(
          (user) => user.username !== newFriendUser,
        );
        setUsersCasted(filteredUsernames);
      } else {
            ...
            <PopoverContent className="p-3">
              <div className="flex flex-col gap-2">
                ...
                <Autocomplete
                  ...
                  className="max-w-xs"
                  defaultItems={usersCasted.map((user) => ({
                    label: user.username,
                    value: user._id,
                  }))}
