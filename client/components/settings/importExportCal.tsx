...
const ImportExportCal: React.FC<ImportExportCalProps> = ({ events }) => {
  ...
  const renderEventsList = (titles: string[]) => (
    <ScrollShadow className="max-h-[60vh]">
      <div className="text-success font-bold mb-4 border-2 bodrder-black rounded-[20px] p-6">
        Importazione Completata
        <p className=" text-lg text-black dark:text-white font-semibold mb-4">
          Sono stati importati {titles.length} eventi:
        </p>
  ...
  const exportToIcal = () => {
    ...
    showModal(
      ...
      <div className="text-center py-4">
        <p>Esportazione del calendario avvenuta con successo</p>
        <p className="text-sm text-gray-600 mt-2">È stato creato un file my-calendar.ics</p>
      </div>
    );
  };
